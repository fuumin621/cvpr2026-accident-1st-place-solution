#!/usr/bin/env python3
"""exp160b: 32B three-stage VLM pipeline.

Stage 1 runs Qwen3-VL-32B-Instruct-FP8 on the full clip (4 fps,
<=128 frames, 960 px longest side; two-pass split for clips >32 s)
to produce a joint (time, position, type) prediction. Stage 2 calls
the same VLM on a denser window around the predicted time and blends
the refined time back with a small, clipped correction. Stage 3
extracts a single frame at the refined time and asks the VLM to point
at the collision; that point replaces the Stage-1 coordinates. A
rule-based type post-fix runs last.
"""
...
class Exp160bPredictor:
    ...
    def _run_stage1_pass(
        ...
        with tempfile.TemporaryDirectory(prefix=f"exp160b_s1_{pass_label}_") as td:
    ...
    def build_stage1_prefetch(self, video_path: Path, video_rel: str = "") -> dict[str, Any]:
        ...
        cache_dir = Path(tempfile.mkdtemp(prefix="exp160b_prefetch_"))
    ...
    def _run_time_refinement(
        ...
        with tempfile.TemporaryDirectory(prefix="exp160b_trefine_") as td:
    ...
    def _run_stage3_grounding(
        ...
        with tempfile.TemporaryDirectory(prefix="exp160b_s3_") as td:
